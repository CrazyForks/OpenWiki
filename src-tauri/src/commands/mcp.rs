...
pub async fn copy_content_summary(
    ...
    if contents.is_empty() {
        return Ok("最近 7 天没有保存的内容。".to_string());
    }

    let mut lines = Vec::new();
    lines.push(format!("以下是我最近 7 天保存的内容（共 {} 条）：\n", contents.len().min(50)));

    for (i, item) in contents.iter().take(50).enumerate() {
        let date = &item.captured_at[..10];
        let source = &item.source_app;
        let preview = item.raw_text.as_deref()
            .or(item.source_url.as_deref())
            .unwrap_or("[图片]");
        let preview_short: String = preview.chars().take(150).collect();
        lines.push(format!("{}. [{}] 来自 {}: {}", i + 1, date, source, preview_short));
    }

    if contents.len() > 50 {
        lines.push(format!("\n...还有 {} 条未显示", contents.len() - 50));
    }
